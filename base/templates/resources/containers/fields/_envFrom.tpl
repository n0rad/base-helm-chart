...
*/}}
{{- define "base.resources.containers.fields.envFrom" -}}
  {{- $ctx := .ctx -}}
  ...
  {{- if not (empty (get $containerObject "envFrom")) -}}
    ...
    {{- range $containerObject.envFrom -}}
      ...
      {{- if hasKey . "configMap" -}}
        {{- $configMap := include "base.resources.configMaps.getByIdentifier" (dict "rootContext" $rootContext "id" .configMap) | fromYaml -}}
      ...
      {{- else if hasKey . "configMapRef" -}}
        {{- if not (empty (dig "identifier" nil .configMapRef)) -}}
          {{- $configMap := include "base.resources.configMaps.getByIdentifier" (dict "rootContext" $rootContext "id" .configMapRef.identifier) | fromYaml -}}
          {{- if empty $configMap -}}
      ...
      {{- else if hasKey . "secret" -}}
        {{- $secret := include "base.resources.sealedSecrets.getByIdentifier" (dict "rootContext" $rootContext "id" .secret) | fromYaml -}}
      ...
      {{- else if hasKey . "secretRef" -}}
        {{- if not (empty (dig "identifier" nil .secretRef)) -}}
          {{- $secret := include "base.resources.sealedSecrets.getByIdentifier" (dict "rootContext" $rootContext "id" .secretRef.identifier) | fromYaml -}}
          {{- if empty $secret -}}
