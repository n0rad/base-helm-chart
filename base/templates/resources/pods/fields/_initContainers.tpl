...
*/ -}}
{{- define "base.resources.pods.fields.initContainers" -}}
  ...
  {{- $containersWithDependsOn := include "base.lib.getMapItemsWithKey" (dict "map" $controllerObject.initContainers "key" "dependsOn") | fromYaml | keys -}}
  {{- $useDependsOn := gt (len $containersWithDependsOn) 0 -}}

  {{- range $key, $containerValues := $controllerObject.initContainers -}}
    {{- if include "base.lib.utils.isEnabled" $containerValues -}}
